...
function [shift1 shift2 shift3]=driftcorrection_core3D(im1,im2,iniguess)
kernelsize=2;  
if isempty(im1)||isempty(im2)||(sum(im1(:))==0)||(sum(im2(:))==0)
    ...
    corrim3d=normxcorr3_sparse(imgaussfilt3(single(im1),kernelsize),imgaussfilt3(single(im2),kernelsize),'same');
