...
classdef CalCRLB < handle
    ...
    methods
        ...
        function prepInputparam(obj)
            ...
            bg=[];
            z=[];
            x0=cat(2,obj.Xpos,obj.Ypos,obj.Zpos,obj.Photon,obj.Bg);
            for t=1:pN
                x = cat(2,x,x0(:,1)+PIx0(t));
                y = cat(2,y,x0(:,2)+PIy0(t));
                z = cat(2,z,x0(:,3)+PIz0(t));
                I = cat(2,I,x0(:,4).*PIi(t)+PIi0(t));
                bg = cat(2,bg,x0(:,5).*PIi(t));
            end
            obj.Xin=cat(2,reshape(x',N*pN,1),reshape(y',N*pN,1),reshape(z',N*pN,1),reshape(I',N*pN,1),reshape(bg',N*pN,1));
        ...
        function calcrlb(obj)
            ...
            for s=0:N-1
                ...
                %z
                funFi(:,:,3)=obj.Photon(t).*(psf(:,:,s*pN+5)-psf(:,:,s*pN+1))./obj.Deltaz;
                %I
                funFi(:,:,4)=psf(:,:,s*pN+2);
                %bg
                funFi(:,:,5)=1;
