...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @INSPR_GUI_OpeningFcn, ...
                   'gui_OutputFcn',  @INSPR_GUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before INSPR_GUI is made visible.
function INSPR_GUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to INSPR_GUI (see VARARGIN)

% Choose default command line output for INSPR_GUI
...
data_empupil.setup.biplane_dist = 0.298; %um
...
data_empupil.pupil.Zshift_mode = 1; %Z shift mode in each iteration
data_empupil.pupil.Zrange_low = -1.4;
data_empupil.pupil.Zrange_mid = 0.2;
data_empupil.pupil.Zrange_high = 1.4;
data_empupil.pupil.Z_pos = [-1.4:0.14:1.4];
data_empupil.pupil.bin_lowerBound = 30; %the lower bound of each group images
data_empupil.pupil.iter = 8;
data_empupil.pupil.min_similarity = 0.5; %min similarity in NCC calculation
data_empupil.pupil.blur_sigma = 2;

data_empupil.pupil.XYshift_mode = 1;   %In classification, 0 for XY shift separately; 1 for XY shift together
data_empupil.pupil.iter_mode = 0;   
...
function varargout = INSPR_GUI_OutputFcn(hObject, eventdata, handles) 
...
global data_empupil
[filename,pathname] = uigetfile(fullfile(data_empupil.setup.workspace,'*.mat'),'Select the transformation model');
...
if isequal(filename,0)
   ...
   if isfield(tmp,'tform')
       data_empupil.tform = tmp.tform;
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
addpath('.\Biplane_registration\')

global data_empupil

disp('Calculate the transformation model...')

if isfield(data_empupil,'qd1') && isfield(data_empupil,'qd2')
    tform = biplane_registration(data_empupil.qd1,data_empupil.qd2);
    data_empupil.tform = tform;
...
[filename, pathname] = uiputfile(fullfile(data_empupil.setup.workspace,'*.mat'), 'save the transformation model');
if isequal(filename,0) || isequal(pathname,0)
   disp('User selected Cancel')
else
   disp(['User selected ',fullfile(pathname,filename)])

   tform = data_empupil.tform;
   save(fullfile(pathname,filename),'tform');
end
...
% handles    structure with handles and user data (see GUIDATA)
global data_empupil
% [filename,pathname] = uigetfile('*.mat','Select the Biplane data');
[filename,pathname] = uigetfile(fullfile(data_empupil.setup.workspace,'*.mat'),'Select the Biplane data');
...
if isequal(filename,0)
   ...
   if isfield(tmp,'qd1') && isfield(tmp,'qd2')    
       if (size(tmp.qd1,1) == size(tmp.qd2,1) && size(tmp.qd1,2) == size(tmp.qd2,2) && size(tmp.qd1,3) == size(tmp.qd2,3))
           %if import a new data. Make ‘Show merge’ in Biplane
           %registration and ‘Run’, ‘Show subregion’ and ‘Export’ in
           %Segmentation disable
           if isfield(data_empupil,'qd1')
...
% fnum_display = size(data_empupil.qd1,3);
% if fnum_display > 100
%     fnum_display = 100;
% end
% disp('Show two planes data...')
% h1 = dipshow(data_empupil.qd1(:,:,1:fnum_display));
% h1.Name = 'Channel 1';
% h2 = dipshow(data_empupil.qd2(:,:,1:fnum_display));
% h2.Name = 'Channel 2';

disp('Show two planes data...')
figure; imshow(max(data_empupil.qd1,[],3),[]);
axis tight
title('Plane 1');

figure; imshow(max(data_empupil.qd2,[],3),[]);
axis tight
title('Plane 2');
...
% h = joinchannels('RGB', data_empupil.qd1(:,:,1:fnum_display), data_empupil.qd2(:,:,1:fnum_display));
% dipshow(h);
disp('Show merge data...')

figure; imshowpair(max(data_empupil.qd1,[],3), max(data_empupil.qd2,[],3),'ColorChannels',[1 2 0], 'Scaling','joint');
axis tight
title('Merged image');
...
if isequal(filename,0)
   ...
   if isfield(tmp,'setup')
       ...
       if isfield(tmp.setup,'biplane_dist')
           data_empupil.setup.biplane_dist = tmp.setup.biplane_dist;
       else
           msgbox('No Biplane distance. Use the default!');
       end
...
h_biplane_dist  = uicontrol(f_set, 'Style','edit','String',num2str(data_empupil.setup.biplane_dist),...
    'FontSize',10,'Position',[200 550 150 25]);
...
%save figure handle
h_set.h_biplane_dist = h_biplane_dist;
...
if isequal(filename,0)
   ...
   if isfield(sCMOS_input,'ccdoffset_ch1') && isfield(sCMOS_input,'ccdoffset_ch2') && ...
           isfield(sCMOS_input,'ccdvar_ch1') && isfield(sCMOS_input,'ccdvar_ch2') && ...
           isfield(sCMOS_input,'gain_ch1') && isfield(sCMOS_input,'gain_ch2')
...
%update data_empupil
data_empupil.setup.biplane_dist = default_cfg.setup.biplane_dist;
...
%update UI
set(t.h_biplane_dist,'String', num2str(data_empupil.setup.biplane_dist));
...
biplane_dist = str2num( get(t.h_biplane_dist,'String') ); 
...
data_empupil.setup.biplane_dist = biplane_dist;
...
% show_ch2 = [];
% 
% for i = 1 : size(data_empupil.subregion_ch1,3)
%     tmp1 = data_empupil.subregion_ch1(:,:,i);
%     show_ch1(:,:,i) = tmp1./max(tmp1(:));
%     tmp2 = data_empupil.subregion_ch2(:,:,i);
%     show_ch2(:,:,i) = tmp2./max(tmp2(:));
% end
% 
% % show PSFs
% col_img = ones(size(data_empupil.subregion_ch1,1),2,size(data_empupil.subregion_ch1,3));
% 
% PSFs_all = cat(2, show_ch1,col_img,show_ch2);
% h_subregion = dipshow(PSFs_all);
% diptruesize(h_subregion,800)
% h_subregion.Name = 'PSFs in two channles';
...
if isfield(data_empupil,'qd2_registered') 
    ...
    [subregion_ch1,subregion_ch2,seg_display] = crop_subregion(data_empupil.qd1,data_empupil.qd2,data_empupil.tform,boxsz,thresh,thresh_dist,data_empupil.setup);

    data_empupil.subregion_ch1 = subregion_ch1;
    data_empupil.subregion_ch2 = subregion_ch2;
...
%     set(handles.seg_subregion_pushbutton,'Enable','on');
    ...
    msgbox('Please process biplane registration!');
end
...
global data_empupil
[filename,pathname] = uigetfile(fullfile(data_empupil.setup.workspace,'*.mat'),'Select the Biplane subregion images..');
...
%    set(handles.pupil_import_edit,'String',[pathname filename]);
   set(handles.pupil_import_edit,'String',filename);

   % load file
   tmp = load([pathname filename]);
   
   if isfield(tmp,'subregion_ch1') && isfield(tmp,'subregion_ch2')
       if (size(tmp.subregion_ch1,1) == size(tmp.subregion_ch2,1) && size(tmp.subregion_ch1,2) == size(tmp.subregion_ch2,2) ...
               && size(tmp.subregion_ch1,3) == size(tmp.subregion_ch2,3))
           data_empupil.subregion_ch1 = tmp.subregion_ch1;
           data_empupil.subregion_ch2 = tmp.subregion_ch2;
...
global data_empupil
[filename,pathname] = uigetfile(fullfile(data_empupil.setup.workspace,'*.mat'),'Select the Biplane data (you can choose multiple data)...','MultiSelect','on');
...
if isequal(filename,0)
   ...
   msgbox({ 'Finish importing the Biplane data path!' ['You choose ' num2str(dirN) ' data'] });
...
if isequal(filename,0)
    ...
    %    set(handles.recon_import_tform_edit,'String',[pathname filename]);
...
if data_empupil.recon.isNewdata == 0
    ...
    data_empupil.recon.qd1 = data_empupil.qd1;
    data_empupil.recon.qd2 = data_empupil.qd2;
...
% save backup
% writeim(colorim,fullfile(data_empupil.setup.workspace,['SR_Zcol_' flagstr '_backup']),'png');
...
if ~isfield(data_empupil,'subregion_ch1') || ~isfield(data_empupil,'subregion_ch2')
...
% data_empupil.pupil.blur_sigma = blur_sigma;

%normalization for subregion images
display('Image normalization...');

[subregion_ch1_norm, subregion_ch2_norm] = subregion_normalization(data_empupil.subregion_ch1,data_empupil.subregion_ch2);

%EMpupil process... unfinished
display('EMpupil process...');


probj = INSPR_model_generation(subregion_ch1_norm,subregion_ch2_norm,data_empupil.setup,data_empupil.pupil); %this function is initial, need change
...
[filename, pathname] = uiputfile(fullfile(data_empupil.setup.workspace,'*.mat'), 'save biplane subregion images');
if isequal(filename,0) || isequal(pathname,0)
   disp('User selected Cancel')
else
   disp(['User selected ',fullfile(pathname,filename)])
   ...
   subregion_ch2 = data_empupil.subregion_ch2;

   save(fullfile(pathname,filename),'subregion_ch1','subregion_ch2');
end
...
if get(handles.pupil_zernike_checkbox,'Value')==1
    ...
    data_empupil.pupil.init_z = zeros(1,21);    %empty initial pupil
...
if isfield(data_empupil,'qd1') && isfield(data_empupil,'qd2') && isfield(data_empupil,'tform')
    display('Registrate Biplane data...');
    
    rate = mean(mean(mean(data_empupil.qd1))) / mean(mean(mean(data_empupil.qd2)));
    data2 = data_empupil.qd2 * rate;  
    %cubic
    data_empupil.qd2_registered = imwarp(data2,data_empupil.tform,'cubic','OutputView',imref2d(size(data_empupil.qd1)));
    set(handles.registration_merge_pushbutton,'Enable','on');
    
    %Enable
    set(handles.seg_process_pushbutton,'Enable','on');
    msgbox('Finish Biplane registration!');
else
...
% end
% 
% h = joinchannels('RGB', data_empupil.qd1(:,:,1:fnum_display), data_empupil.qd2_registered(:,:,1:fnum_display));
% h_merge = dipshow(h);
% h_merge.Name = 'After registration';

disp('Show registrated data...')
figure; imshowpair(max(data_empupil.qd1,[],3), max(data_empupil.qd2_registered,[],3),'ColorChannels',[1 2 0], 'Scaling','joint');
axis tight
...
switch contents{get(handles.pupil_xyshift_mode_popupmenu,'Value')}
    case 'Separate shift'
        data_empupil.pupil.XYshift_mode = 0;   
    case 'Together shift'
        data_empupil.pupil.XYshift_mode = 1;
    otherwise
        data_empupil.pupil.XYshift_mode = 1;
end

display(['XY shift mode is: ' num2str(data_empupil.pupil.XYshift_mode)]);
