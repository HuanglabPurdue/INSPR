...
void Model_Localization::setup(){
	...
	cudaMalloc(&d_tMat, 6*sizeof(float));	//affine without translation; 3x2=6 matrix
	cudaMalloc(&d_offset_int, 2*Nfit*sizeof(float));	//interger offset
	...
	cudaMemcpy(d_tMat, tMat, 6*sizeof(float), cudaMemcpyHostToDevice);	//Affine parameters
	cudaMemcpy(d_offset_int, offset_int, 2 * Nfit*sizeof(float), cudaMemcpyHostToDevice);
...
void Model_Localization::cleanUp(){
	...
	cudaFree(d_tMat);	//affine parameters
	cudaFree(d_offset_int);
	CudaError("Cuda Free");
...
void Model_Localization::mLocalization(){
	...
	for (int s = 0; s < NCH; s++)		//Edited by FX
	{
		...
		kernel_calcPSFPixel_wrapper(dimGrid, dimBlock, &d_SampledPSF[s*Samplepsf_Pixels], &d_SamplePSFx[s*Samplepsf_Pixels], &d_SamplePSFy[s*Samplepsf_Pixels], &d_SamplePSFz[s*Samplepsf_Pixels],
			...
			SampleSpacingXY, SampleSpacingZ, StartX, StartY, StartZ, N_int, PSFSizeOut, Nfit, d_tMat, d_offset_int, s);
		CudaError("mLocalization::kernel_calcPSFPixel_wrapper");

		kernel_getdev_wrapper(dimGrid2, dimBlock2, &d_Data[s*Nfit*PSFSize], d_PSFs, d_dPSFx, d_dPSFy, d_dPSFz, 
...
void Model_Localization::calCRLB(){
	...
	for (int s = 0; s < NCH; s++)		//Edited by FX
	{
		...
		kernel_calFisherM_wrapper(dimGrid2, dimBlock2, d_PSFs, d_dPSFx, d_dPSFy, d_dPSFz, &d_I[s*Nfit], &d_Bg[s*Nfit], &d_ParamF[s*NPL*NPL*Nfit],
			s, Nfit, PSFSize, d_tMat);	//Fisher information need consider affine
...
void Model_Localization::calErr(){
	...
	for (int s = 0; s < NCH; s++)		//Edited by FX
	{
		...
		kernel_Err_wrapper(dimGrid2, dimBlock2, d_PSFI, &d_Data[s*Nfit*PSFSize], d_Error, Nfit, PSFSize);
	}
